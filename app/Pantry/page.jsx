...
import { useSession } from "next-auth/react";
import { redirect } from "next/navigation";
import { encodeUserIdentifier } from '../utils/encodeUserIdentifier'; // Import the encoding function
...
function Home() {
  ...
  const userCollection = session?.user ? encodeUserIdentifier(session.user.email, session.user.role) : '';

  const updateInventory = async () => {
    const snapshot = query(collection(firestore, `inventory-${userCollection}`)); // Use unique collection
    const docs = await getDocs(snapshot);
    const inventoryList = [];
    docs.forEach((doc) => {
      inventoryList.push({ name: doc.id, ...doc.data() });
  ...
  const addItem = async (item, category, description) => {
    const docRef = doc(collection(firestore, `inventory-${userCollection}`), item);
    const docSnap = await getDoc(docRef);
    if (docSnap.exists()) {
      const { quantity } = docSnap.data();
      await setDoc(docRef, { quantity: quantity + 1, category, description });
